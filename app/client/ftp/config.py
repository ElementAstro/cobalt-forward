...
class FTPConfig:
    ...
    passive_ports: List[int] = field(
        default_factory=lambda: list(range(60000, 61000)))
    ...
    def _validate_config(self) -> None:
        ...
        if self.port < 1 or self.port > 65535:
            raise ValueError(
                f"Invalid port: {self.port}. Must be between 1 and 65535.")

        # Validate timeout
        if self.timeout < 1:
            raise ValueError(
                f"Invalid timeout: {self.timeout}. Must be positive integer.")

        # Validate TLS parameters
        if self.use_tls:
            if self.cert_file and not os.path.exists(self.cert_file):
                raise ValueError(
                    f"Certificate file not found: {self.cert_file}")
            if self.key_file and not os.path.exists(self.key_file):
                raise ValueError(f"Key file not found: {self.key_file}")

    @classmethod
    def from_json(cls: Type[T], json_file: str) -> T:
    ...
    @classmethod
    def create_default(cls: Type[T], host: str, save_path: Optional[str] = None) -> T:
    ...
    def update(self, **kwargs: Any) -> 'FTPConfig':
    ...
    @classmethod
    def merge(cls: Type[T], *configs: 'FTPConfig') -> T:
    ...
    def _validate_config(self) -> None:
        ...
        if self.require_tls:
            if not self.tls_certfile or not self.tls_keyfile:
                raise ValueError(
                    "TLS certificate and key files are required when TLS is mandatory")
            if not os.path.exists(self.tls_certfile):
                raise ValueError(
                    f"TLS certificate file not found: {self.tls_certfile}")
        ...
        if self.anonymous_enable and self.anonymous_root:
            if not os.path.exists(self.anonymous_root):
                try:
                    ...
                    raise ValueError(
                        f"Cannot create anonymous root directory: {self.anonymous_root}")
        ...
        if not os.path.exists(self.user_root):
            try:
                ...
                raise ValueError(
                    f"Cannot create user root directory: {self.user_root}")

    @classmethod
    def create_default_server(cls, address: str = "0.0.0.0", port: int = 21, save_path: Optional[str] = None) -> 'FTPServerConfig':
