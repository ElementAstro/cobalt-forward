...
class SSHPlugin(ABC):
    ...
    async def pre_execute_command(self, command: str, timeout: Optional[int] = None) -> Optional[tuple[str, Optional[int]]]:
        """执行命令前预处理
        ...
        Returns:
            Optional[tuple[str, Optional[int]]]: 处理后的命令和超时时间
            若返回 None，则阻止执行
        """
        return command, timeout

    async def post_execute_command(self, _command: str, result: Dict[str, Any]) -> Dict[str, Any]:
        """执行命令后处理
    ...
    async def pre_upload_file(self, local_path: str, remote_path: str) -> Optional[tuple[str, str]]:
        """上传文件前预处理
        ...
        Returns:
            Optional[tuple[str, str]]: 处理后的本地路径和远程路径
            若返回 None，则阻止上传
        """
        return local_path, remote_path

    async def post_upload_file(self, _local_path: str, _remote_path: str, _success: bool) -> None:
        """上传文件后处理
    ...
    async def pre_download_file(self, remote_path: str, local_path: str) -> Optional[tuple[str, str]]:
        """下载文件前预处理
        ...
        Returns:
            Optional[tuple[str, str]]: 处理后的远程路径和本地路径
            若返回 None，则阻止下载
        """
        return remote_path, local_path

    async def post_download_file(self, _remote_path: str, _local_path: str, _success: bool) -> None:
        """下载文件后处理
    ...
    async def on_error(self, _operation: str, _error: Exception) -> None:
        """处理错误事件

        Args:
            _operation: 发生错误的操作名称
            _error: 发生的异常
        """
        pass
...
class SSHPluginManager:
    ...
    async def run_pre_execute_command(self, command: str, timeout: Optional[int] = None) -> tuple[Optional[str], Optional[int]]:
        ...
        Returns:
            ...
            若返回 None，则阻止执行
        """
        current_command: Optional[str] = command
        current_timeout: Optional[int] = timeout
        ...
        for plugin in self._sorted_plugins:
            try:
                ...
                logger.error(
                    f"插件 '{plugin.name}' 的 pre_execute_command 中出错: {e}")
    ...
    async def run_post_execute_command(self, command: str, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for plugin in self._sorted_plugins:
            try:
                ...
                logger.error(
                    f"插件 '{plugin.name}' 的 post_execute_command 中出错: {e}")
    ...
    async def run_pre_upload_file(self, local_path: str, remote_path: str) -> tuple[Optional[str], Optional[str]]:
        ...
        Returns:
            ...
            若返回 None，则阻止上传
        """
        current_local: Optional[str] = local_path
        current_remote: Optional[str] = remote_path
    ...
    async def run_pre_download_file(self, remote_path: str, local_path: str) -> tuple[Optional[str], Optional[str]]:
        ...
        for plugin in self._sorted_plugins:
            try:
                ...
                logger.error(
                    f"插件 '{plugin.name}' 的 pre_download_file 中出错: {e}")
    ...
    async def run_post_download_file(self, remote_path: str, local_path: str, success: bool) -> None:
        ...
        for plugin in self._sorted_plugins:
            try:
                ...
                logger.error(
                    f"插件 '{plugin.name}' 的 post_download_file 中出错: {e}")
