...
class CommandDispatcher(IComponent, ICommandDispatcher):
    ...
    def __init__(self) -> None:
        self._handlers: Dict[Type[Command[Any]], List[ICommandHandler[Any, Any]]] = defaultdict(list)
    ...
    async def register_handler(self, handler: ICommandHandler[Any, Any]) -> None:
        ...
        for command_type in handler.supported_commands:
            ...
            logger.debug(f"Registered handler {handler.__class__.__name__} for command {command_type.__name__}")
        
        self._metrics['handlers_count'] = sum(len(handlers) for handlers in self._handlers.values())
        ...
        for command_type in handler.supported_commands:
            if command_type in self._handlers:
                try:
                    ...
                    removed = True
                    logger.debug(f"Unregistered handler {handler.__class__.__name__} for command {command_type.__name__}")
                except ValueError:
    ...
    def add_validator(self, validator: ICommandValidator) -> None:
        ...
        logger.debug(f"Added command validator: {validator.__class__.__name__}")
    
    def add_interceptor(self, interceptor: ICommandInterceptor) -> None:
        """Add a command interceptor."""
        self._interceptors.append(interceptor)
        logger.debug(f"Added command interceptor: {interceptor.__class__.__name__}")
    ...
    async def _execute_command(self, command: Command[Any]) -> CommandResult[Any]:
        ...
        if not handlers:
            raise ValueError(f"No handler found for command type: {command_type.__name__}")
        ...
        if not best_handler:
            raise ValueError(f"No suitable handler found for command: {command.name}")
