...
class EventBus(IComponent, IEventBus):
    ...
    def __init__(self, max_workers: int = 10, queue_size: int = 1000):
        self._subscriptions: Dict[str,
                                  List[EventSubscription]] = defaultdict(list)
    ...
    async def configure(self, config: Dict[str, Any]) -> None:
        ...
        if max_workers != self._max_workers:
            logger.info(
                f"Updating max workers from {self._max_workers} to {max_workers}")
            self._max_workers = max_workers
    ...
    async def publish(self, event: Union[Event, str], data: Any = None,
        ...
        try:
            ...
            logger.debug(
                f"Published event: {event.name} (ID: {event.event_id})")
    ...
    async def subscribe(self, event_name: str, handler: Callable[[Event], Any],
        ...
        logger.debug(
            f"Added subscription for '{event_name}' (ID: {subscription_id})")
        ...
        for event_name, subscriptions in self._subscriptions.items():
            for i, subscription in enumerate(subscriptions):
                if subscription.subscription_id == subscription_id:
                    ...
                    self._metrics['subscriptions_count'] -= 1
                    logger.debug(
                        f"Removed subscription {subscription_id} for '{event_name}'")
                    return True
