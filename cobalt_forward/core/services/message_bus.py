...
class MessageBus(IComponent, IMessageBus):
    ...
    async def send(self, message: Message) -> None:
        ...
        try:
            ...
            if (transformed_message.message_type == MessageType.RESPONSE and 
                ...
                future = self._pending_requests.pop(transformed_message.correlation_id)
                if not future.done():
    ...
    async def subscribe(self, topic: str, handler: Callable[[Message], Any]) -> str:
        ...
        logger.debug(f"Added subscription for topic '{topic}' (ID: {subscription_id})")
        ...
        for topic, subscriptions in self._subscriptions.items():
            for i, subscription in enumerate(subscriptions):
                if subscription.subscription_id == subscription_id:
                    subscriptions.pop(i)
                    self._metrics['subscriptions_count'] -= 1
                    logger.debug(f"Removed subscription {subscription_id} for topic '{topic}'")
                    return True
    ...
    def add_transformer(self, transformer: IMessageTransformer) -> None:
        ...
        logger.debug(f"Added message transformer: {transformer.__class__.__name__}")
