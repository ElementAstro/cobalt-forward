...
class SSHConnectionPool:
    ...
    async def _create_connection(self) -> Optional[SSHClientConnection]:
        ...
        try:
            ...
            logger.debug(
                f"Created new SSH connection to {self._config.host}:{self._config.port}")
            return connection
...
class SSHClient(BaseClient, ISSHClient):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if test_connection:
                ...
                logger.info(
                    f"SSH client connected to {self._ssh_config.host}:{self._ssh_config.port}")
                return True
            else:
    ...
    async def disconnect(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"SSH client disconnected from {self._ssh_config.host}:{self._ssh_config.port}")
    ...
    @asynccontextmanager
    async def _get_connection(self) -> AsyncGenerator[SSHClientConnection, None]:
        ...
        try:
            async with self._get_connection() as connection:
                ...
                self._metrics.record_request(
                    result.exit_status == 0, execution_time)
        ...
        try:
            async with self._get_connection() as connection:
                async with connection.start_sftp_client() as sftp:
                    ...
                    logger.info(
                        f"File uploaded: {local_path} -> {remote_path}")
        ...
        try:
            async with self._get_connection() as connection:
                async with connection.start_sftp_client() as sftp:
                    ...
                    logger.info(
                        f"File downloaded: {remote_path} -> {local_path}")
    ...
    async def forward_local_port(
        ...
        try:
            async with self._get_connection() as connection:
                ...
                logger.info(
                    f"Local port forward: {local_host}:{local_port} -> {remote_host}:{remote_port}")
                return listener
    ...
    async def forward_remote_port(
        ...
        try:
            async with self._get_connection() as connection:
                ...
                logger.info(
                    f"Remote port forward: {remote_host}:{remote_port} -> {local_host}:{local_port}")
                return listener
