...
class ConfigManager(IComponent):
    ...
    async def enable_hot_reload(self) -> None:
        ...
        if not WATCHDOG_AVAILABLE:
            logger.warning(
                "Hot reload not available: watchdog package not installed")
            return
    ...
    def register_observer(self, observer: Callable[[ApplicationConfig], None]) -> None:
        ...
        if observer not in self._observers:
            ...
            logger.debug(
                f"Registered configuration observer: {observer.__name__}")
    ...
    def unregister_observer(self, observer: Callable[[ApplicationConfig], None]) -> None:
        ...
        if observer in self._observers:
            ...
            logger.debug(
                f"Unregistered configuration observer: {observer.__name__}")
    ...
    async def _reload_config(self) -> bool:
        ...
        try:
            logger.info(
                f"Reloading configuration from {self._config_file_path}")

            # Load new configuration
            new_config = self._config_loader.load_config(
    ...
    async def _notify_observers(self, old_config: ApplicationConfig, new_config: ApplicationConfig) -> None:
        ...
        for observer in self._observers:
            try:
                ...
                logger.error(
                    f"Error in configuration observer {observer.__name__}: {e}")
