...
class PluginManager(IPluginManager):
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self._plugin_context = PluginContext(
                self._container, config_manager, event_bus)
    ...
    async def configure(self, config: Dict[str, Any]) -> None:
        ...
        self._plugin_directory = config.get(
    ...
    async def load_plugin(self, plugin_path: str) -> bool:
        ...
        try:
            ...
            if not await self._check_dependencies(plugin):
                logger.error(
                    f"Plugin dependencies not satisfied: {plugin_name}")
                return False
    ...
    async def discover_plugins(self, directory: str) -> List[str]:
        ...
        logger.info(
            f"Discovered {len(plugin_paths)} potential plugins in {directory}")
        return plugin_paths
