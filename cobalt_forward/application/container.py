...
class Container(IContainer):
    ...
    def register(self, 
                 ...
                 lifetime: ServiceLifetime = ServiceLifetime.SINGLETON) -> None:
        """Register a service with the container."""
        logger.debug(f"Registering service: {service_type.__name__} -> {implementation}")
        ...
        logger.info(f"Registered {service_type.__name__} with {lifetime.name} lifetime")
    ...
    def resolve(self, service_type: Type[T]) -> T:
        ...
        try:
            ...
            raise ServiceResolutionException(f"Failed to resolve {service_type.__name__}: {str(e)}") from e
    ...
    def _analyze_dependencies(self, implementation_class: Type[Any]) -> List[Type[Any]]:
        ...
        try:
            ...
            logger.warning(f"Failed to analyze dependencies for {implementation_class.__name__}: {e}")
            return []
